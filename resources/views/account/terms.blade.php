<div class="space-y-6">
    <x-account::page-header :title="$title" subtitle="Nutzungsbedingungen für die Plattform {{ config('app.name', 'Platform') }}" color="gray">
        <x-slot:icon>
        ...
        <div class="p-4 sm:p-6">
            <div class="prose prose-sm max-w-none dark:prose-invert">
                @if ($content)
                    {!! $content !!}
                @else
                    <p class="text-sm text-gray-500 dark:text-gray-400">Noch keine AGB hinterlegt.</p>
                @endif
            </div>
        </div>
    </div>
